...
client_secrets_json_path = os.environ.get('GOOGLE_AUTH_CLIENT_SECRET_JSON') or os.path.join(pathlib.Path(__file__).parent, 'client_secret.json')
client_secrets_file = client_secrets_json_path # set the path to where the .json file you got Google console is
redirect_uri = os.environ.get('GOOGLE_AUTH_REDIRECT_URI') or 'https://127.0.0.1:5000/login/callback' 

flow = Flow.from_client_secrets_file(  # Flow is OAuth 2.0 a class that stores all the information on how we want to authorize our users
    client_secrets_file=client_secrets_file,
    scopes=['https://www.googleapis.com/auth/userinfo.profile', 'https://www.googleapis.com/auth/userinfo.email', 'openid'],  # here we are specifing what do we get after the authorization
    redirect_uri=redirect_uri # and the redirect URI is the point where the user will end up after the authorization
)
